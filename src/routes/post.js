...
router.get("/", (req, res) => {
  ...
  /*const validateData = validateAndGetPaginationData(inputLimit, inputStart)
  const limit = validateData.limit
  const start = validateData.start
  const errMsg = validateData.errMsg
...
router.post("/", (req, res)=>{
  ...
  postData.push(dataToBeAdded)

  res.status(201).json(dataToBeAdded)
})
